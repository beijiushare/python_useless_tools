...
def process_image(image_path, is_encrypt=True):
    ...
    # 显示进度条
    progress_bar['maximum'] = width * height
    ...
    for i in range(0, width * height, block_size):
        ...
        progress_bar['value'] = min(i + block_size, width * height)  # 更新进度条
...
# 麻了麻了真麻了
""" copy_button = tk.Button(left_frame, text="复制图片", command=copy_image_to_clipboard, font=font_song)
